...
REPO="https://github.com/FacileStudio/nuage-cli.git"
BIN_NAME="nuage"

info()  { printf '\033[1;36m%s\033[0m\n' "$*"; }
error() { printf '\033[1;31merror:\033[0m %s\n' "$*" >&2; exit 1; }

SKILL_MARKER_START="<!-- nuage:start -->"
SKILL_MARKER_END="<!-- nuage:end -->"
...
register_skill() {
  ...
  if command -v claude &>/dev/null; then
    local skill_dir="$HOME/.claude/skills/nuage"
    mkdir -p "$skill_dir"
...
info "Cloning $REPO..."
git clone --depth 1 --quiet "$REPO" "$TMPDIR/nuage-cli"

info "Building and installing $BIN_NAME..."
cargo install --path "$TMPDIR/nuage-cli" --force --quiet

register_skill "$TMPDIR/nuage-cli"
